...
void AgentOrganizer::update(float dt)
{
    ...
    std::vector<Agent*> customers = get_all_agents_by_type("Customer");
    for (size_t i{}; i < customers.size(); ++i)
    {   // For loop to get the smallest queueID
        BehaviorAgent* ba = dynamic_cast<BehaviorAgent*>(customers[i]);
        if (!ba) continue;

        Blackboard& bb = ba->get_blackboard();
        if (bb.get_value<bool>("ordered") || bb.get_value<bool>("leavingStore"))
            continue;

        size_t queueIndex = ba->get_blackboard().get_value<size_t>("queueIndex");
        if (bb.get_value<int>("queueID") < MyVar::lowestID[queueIndex])
            MyVar::lowestID[queueIndex] = bb.get_value<int>("queueID");

    }
    memset(MyVar::moveQ, 0, sizeof(MyVar::moveQ));
}
