...
{
	this->VKConfigs.validation = enableValidation;

	this->VKConfigs.validation = true;
...
void VKAppBase::prepare()
{
	...
	createPipelineCache();
	setupFrameBuffer();
	VKConfigs.overlay = VKConfigs.overlay && (!benchmark.active);
	if (VKConfigs.overlay) {
...
VKAppBase::VKAppBase(bool enableValidation)
{
	...
	VKConfigs.validation = enableValidation;
	...
	if (commandLineParser.isSet("validation")) {
		VKConfigs.validation = true;
	}
	if (commandLineParser.isSet("vsync")) {
		VKConfigs.vsync = true;
	}
...
bool VKAppBase::initVulkan()
{
	...
	// Vulkan instance
	err = createInstance(VKConfigs.validation);
	if (err) {
...
HWND VKAppBase::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
{
	...
	if (VKConfigs.fullscreen)
	{
		if ((width != (uint32_t)screenWidth) && (height != (uint32_t)screenHeight))
		{
			DEVMODE dmScreenVKConfigs;
			memset(&dmScreenVKConfigs, 0, sizeof(dmScreenVKConfigs));
			dmScreenVKConfigs.dmSize = sizeof(dmScreenVKConfigs);
			dmScreenVKConfigs.dmPelsWidth = width;
			dmScreenVKConfigs.dmPelsHeight = height;
			dmScreenVKConfigs.dmBitsPerPel = 32;
			dmScreenVKConfigs.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT;
			if (ChangeDisplayVKConfigs(&dmScreenVKConfigs, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL)
			{
				if (MessageBox(NULL, "Fullscreen Mode not supported!\n Switch to window mode?", "Error", MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
				{
					VKConfigs.fullscreen = false;
				}
				else
				{
					return nullptr;
	...
	windowRect.left = 0L;
	windowRect.top = 0L;
	windowRect.right = VKConfigs.fullscreen ? (long)screenWidth : (long)width;
	windowRect.bottom = VKConfigs.fullscreen ? (long)screenHeight : (long)height;
...
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
int32_t VKAppBase::handleAppInput(struct android_app* app, AInputEvent* event)
{
	VKAppBase* vulkanExample = reinterpret_cast<VKAppBase*>(app->userData);
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION)
	{
		...
		case AINPUT_SOURCE_TOUCHSCREEN: {
			...
			case AMOTION_EVENT_ACTION_MOVE: {
				bool handled = false;
				if (vulkanExample->VKConfigs.overlay) {
					ImGuiIO& io = ImGui::GetIO();
...
void VKAppBase::handleAppCommand(android_app* app, int32_t cmd)
{
	...
	void* displayLinkContext)
{
	@autoreleasepool
	{
		auto vulkanExample = static_cast<VKAppBase*>(displayLinkContext);
...
void* VKAppBase::setupWindow(void* view)
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
...
void VKAppBase::displayLinkOutputCb()
{
	if (prepared)
		nextFrame();
}

void VKAppBase::mouseDragged(float x, float y)
{
	handleMouseMove(static_cast<uint32_t>(x), static_cast<uint32_t>(y));
}

void VKAppBase::windowWillResize(float x, float y)
{
	resizing = true;
	if (prepared)
	{
...
void VKAppBase::windowDidResize()
{
	resizing = false;
}
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
IDirectFBSurface* VKAppBase::setupWindow()
{
	DFBResult ret;
	int posx = 0, posy = 0;
...
void VKAppBase::handleEvent(const DFBWindowEvent* event)
{
	switch (event->type)
	{
	case DWET_CLOSE:
	...
	case DWET_KEYDOWN:
		...
		case KEY_F1:
			if (VKConfigs.overlay) {
				VKConfigs.overlay = !VKConfigs.overlay;
			}
			break;
		default:
			break;
...
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
/*static*/void VKAppBase::registryGlobalCb(void* data,
	wl_registry* registry, uint32_t name, const char* interface,
	uint32_t version)
{
	VKAppBase* self = reinterpret_cast<VKAppBase*>(data);
	self->registryGlobal(registry, name, interface, version);
}

/*static*/void VKAppBase::seatCapabilitiesCb(void* data, wl_seat* seat,
	uint32_t caps)
{
	VKAppBase* self = reinterpret_cast<VKAppBase*>(data);
...
/*static*/void VKAppBase::pointerEnterCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface,
	wl_fixed_t sx, wl_fixed_t sy)
{
}

/*static*/void VKAppBase::pointerLeaveCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface)
{
}

/*static*/void VKAppBase::pointerMotionCb(void* data,
	wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	VKAppBase* self = reinterpret_cast<VKAppBase*>(data);
	self->pointerMotion(pointer, time, sx, sy);
}
void VKAppBase::pointerMotion(wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	handleMouseMove(wl_fixed_to_int(sx), wl_fixed_to_int(sy));
}

/*static*/void VKAppBase::pointerButtonCb(void* data,
...
void VKAppBase::pointerButton(struct wl_pointer* pointer,
...
/*static*/void VKAppBase::pointerAxisCb(void* data,
...
void VKAppBase::pointerAxis(wl_pointer* pointer, uint32_t time,
...
/*static*/void VKAppBase::keyboardKeymapCb(void* data,
	struct wl_keyboard* keyboard, uint32_t format, int fd, uint32_t size)
{
}

/*static*/void VKAppBase::keyboardEnterCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface, struct wl_array* keys)
{
}

/*static*/void VKAppBase::keyboardLeaveCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface)
{
}

/*static*/void VKAppBase::keyboardKeyCb(void* data,
...
void VKAppBase::keyboardKey(struct wl_keyboard* keyboard,
...
/*static*/void VKAppBase::keyboardModifiersCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial, uint32_t mods_depressed,
	uint32_t mods_latched, uint32_t mods_locked, uint32_t group)
{
}

void VKAppBase::seatCapabilities(wl_seat* seat, uint32_t caps)
{
...
void VKAppBase::registryGlobal(wl_registry* registry, uint32_t name,
...
/*static*/void VKAppBase::registryGlobalRemoveCb(void* data,
	struct wl_registry* registry, uint32_t name)
{
}

void VKAppBase::initWaylandConnection()
{
...
void VKAppBase::setSize(int width, int height)
{
	if (width <= 0 || height <= 0)
		return;
...
struct xdg_surface* VKAppBase::setupWindow()
{
...
xcb_window_t VKAppBase::setupWindow()
{
...
void VKAppBase::initxcbConnection()
{
...
void VKAppBase::handleEvent(const xcb_generic_event_t* event)
{
...
{
	swapChain.create(&width, &height, VKConfigs.vsync);
}
