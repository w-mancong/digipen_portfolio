...
GLboolean GLHelper::keystateP = GL_FALSE, GLHelper::mousestateLeft = GL_FALSE;
...
void GLHelper::key_cb(GLFWwindow *pwin, int key, int scancode, int action, int mod) {
  ...
  if (GLFW_PRESS == action) 
  {
      ...
      keystateP = (GLFW_KEY_P == key) ? GLFW_TRUE : GLFW_FALSE;
  }
  else if (GLFW_REPEAT == action)
  {
      // keystate was and is being pressed
      keystateP = GL_FALSE;
  }
  else if (GLFW_RELEASE == action)
  {
...
void GLHelper::mousebutton_cb(GLFWwindow *pwin, int button, int action, int mod) 
{
  ...
  if (GLFW_PRESS == action)
  {
      if(GLFW_MOUSE_BUTTON_LEFT == button)
        mousestateLeft = (GLFW_MOUSE_BUTTON_LEFT == button) ? GLFW_TRUE : GLFW_FALSE;
  }
  else if (GLFW_REPEAT == action)
  {
      // keystate was and is being pressed
