...
//		ofbx::LoadFlags::IGNORE_MESHES |
	...
	int const meshCount = g_scene->getMeshCount();
	for (int i{}; i < meshCount; ++i)
	{
		ofbx::Mesh const* submesh = g_scene->getMesh(i);
		int const matCount = submesh->getMaterialCount();
		for (int j{}; j < matCount; ++j)
		{
			ofbx::Material const* mat =  submesh->getMaterial(j);
			for (int k{}; k < ofbx::Texture::TextureType::COUNT; ++k)
			{
				ofbx::Texture::TextureType const type = static_cast<ofbx::Texture::TextureType>(k);
				ofbx::Texture const* texture = mat->getTexture( type );
				if (!texture)
					continue;
				char fileName[256]{};
				texture->getRelativeFileName().toString(fileName);
				std::cout << "texture type: " << type << " filePath: " << fileName << std::endl;
			}
			std::cout << std::endl;
		}
	}
	...
	if(!g_scene) {
        OutputDebugString((LPCWSTR)ofbx::getError());
    }
    else {
	...
	SetWindowText(hwnd, (LPCWSTR)s_TimeString);
	return true;
}
